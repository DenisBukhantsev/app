...
our_path  = Path.cwd() # тут я сделал переменую что бы он определил директорию где лежит наш модуль, и передать ее в нашу функцию 
def sort_FILES(args): 
  ...
  for file in args.iterdir(): # функция итер дир дает путь ко всем файлам в директории с циклом перебираем все файлы 
    for suff in direct_format: # тут просто идем по словарю
       if file.suffix.lower()[1:] in direct_format[suff]:   # суффикс дает именно формат файла из счетчика файл с путем к файлу .pdf или .avi
         result = args / suff   #тут мы создаем директорию в переменной 
